...
update() {
    ...
    for PR in $PR_LIST; do
        ...
        github_pr=(
            scroll_texts=on
            width=270
            icon="$MERGEABLE_ICON $TITLE ${REVIEWS:-" "} "
    ...
    PREV_REVIES=$(sketchybar --query github.bell | jq -r .label.value)
    ...
    args+=(--set    github.bell                                               \
                    icon=" $OPEN_PULL_REQUESTS  $TOTAL_REVIEWS"                              \
    ...
    if [ $TOTAL_REVIEWS -gt $PREV_REVIEWS ] 2>/dev/null || [ "$SENDER" = "forced" ]; then
      sketchybar --animate tanh 15 --set github.bell icon.y_offset=5 icon.y_offset=0
